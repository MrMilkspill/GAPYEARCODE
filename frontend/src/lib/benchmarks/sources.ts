...
export const benchmarkMethodNotes = [
  ...
  {
    title: "Experience ranges are planning heuristics",
    detail:
  ...
  },
  {
    ...
    detail:
      "AMCAS separates clinical experience into volunteer clinical and paid clinical categories. In this version of the app, volunteer clinical hours set the primary clinical benchmark, while paid clinical work remains valid supporting clinical context instead of being merged into the volunteer subtotal.",
  },
  {
    title: "The heuristic uses a transparent weight order",
    detail:
      "This app weights academics most heavily, followed by volunteer clinical exposure and non-clinical service. Shadowing and research stay lighter in this model because AAMC guidance treats shadowing as more substitutable and research expectations as mission-dependent.",
  },
  {
  ...
  {
    title: "Letter grading uses a conservative common pattern",
    detail:
      "AAMC says letter requirements vary by school. This version therefore uses a conservative common pattern: a committee letter or about two science-faculty letters plus one to two added letters from non-science, research, or clinical/service supervisors.",
  },
] as const;
...
export const benchmarkDerivationNotes: BenchmarkDerivationNote[] = [
  ...
  },
  {
    ...
    currentBands:
      "Volunteer clinical hours are benchmarked separately from paid clinical hours, with volunteer clinical serving as the primary planning benchmark and paid clinical serving as supporting context. For non-clinical service, the model uses planning ranges centered on sustained, community-facing involvement rather than the AAMC class-wide average.",
    rationale:
      "These ranges are transparent planning heuristics informed by AAMC data, AMCAS activity categories, and university advising guidance. They are not official national cutoffs. UVA advising suggests roughly 100 to 200 hours of clinical volunteering or work, AMCAS separates volunteer clinical activity from paid clinical employment, and AAMC's 2025 MD matriculant data reports 717 average community-service hours across the whole matriculant class. That AAMC figure is descriptive mean data, not a required target, and it is influenced by some very high-hour applicants. The app therefore treats quality, consistency, and impact of service as more important than chasing the mean.",
  },
  {
    category: "Shadowing and research",
    currentBands:
      "Shadowing stays low-weight and is tracked within roughly a 40 to 80 hour planning range. Research weight increases only for research-heavy school preferences.",
    rationale:
      "AAMC guidance makes clear that shadowing can be supplemented by other clinical exposure, respected university advising pages usually cluster shadowing around roughly 20 to 50 hours, and research expectations vary by school mission. The app therefore treats shadowing as a bounded planning band with diminishing returns rather than a category where more hours always help.",
  },
  {
  ...
  },
  {
    ...
    currentBands:
      "Committee letter or packet is strongest. Without that, the model looks for a conservative common pattern of about 2 science-faculty letters plus 1 to 2 additional letters from non-science faculty, research mentors, or clinical/service supervisors.",
    rationale:
      "AAMC says letter requirements vary by school, so the app does not claim one universal formula. Instead it uses AAMC guidance plus multiple advising sources that converge on two science letters as a conservative common pattern, then treats additional academic or supervisor letters as the next layer of strength.",
  },
];
...
export const benchmarkSources: BenchmarkSource[] = [
  ...
  },
  {
    ...
    keyStats: [
      ...
      "Average community service across matriculants: 717 hours.",
      "That 717-hour figure is a class-wide mean, not a requirement, cutoff, or expected minimum for individual applicants.",
      "Median undergraduate GPA for matriculants: 3.87.",
    ],
    note:
      "Class-wide means can be pulled upward by applicants with very high totals, including some with gap years or full-time service roles. The app does not treat 717 hours as an individual target.",
  },
  {
  ...
  },
  {
    ...
    note:
      "AMCAS separates clinical experience into volunteer clinical and paid clinical categories. Paid clinical experience is not included in the volunteer-clinical subtotal, but it remains a valid and meaningful form of clinical exposure reviewed by admissions committees.",
  },
  {
