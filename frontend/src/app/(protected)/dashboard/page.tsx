...
export default function DashboardPage() {
  ...
  const [profiles, setProfiles] = useState<SavedPremedProfile[]>([]);
      ...
      try {
        ...
        setError(
          caughtError instanceof Error
            ? caughtError.message
            : "Unable to load your saved profiles.",
        );
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  const scoredProfiles = profiles
    .filter(
      (profile): profile is SavedPremedProfile & {
        scoreResult: NonNullable<SavedPremedProfile["scoreResult"]>;
      } => Boolean(profile.scoreResult),
    )
    .map((profile) => ({
      profile,
      score: profile.scoreResult,
    }));
