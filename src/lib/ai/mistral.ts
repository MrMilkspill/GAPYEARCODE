...
const aiAnalysisSectionSchema = z.object({
  title: z.string().min(1),
  body: stringOrStringArraySchema,
  comparisonIds: z.array(z.string().min(1)).max(3).default([]),
});
...
export const aiProfileAnalysisSchema = z.object({
  ...
  deepDiveSections: z.array(aiAnalysisSectionSchema).min(3).max(5),
...
function buildPromptInput(
  ...
  return {
    ...
    sourceBackedComparisons: comparisons.map((comparison) => ({
      id: comparison.id,
      label: comparison.label,
      evidenceType: comparison.evidenceType,
      applicantValue: comparison.applicantValue,
      benchmarkFact: comparison.benchmarkFact,
...
function normalizeDeepDiveSections(
  sections: z.infer<typeof aiAnalysisSectionSchema>[],
  comparisons: AiSourceBackedComparison[],
) {
  const validComparisonIds = new Set(comparisons.map((comparison) => comparison.id));
  const fallbackId = comparisons[0]?.id ?? "md-academics";

  return sections.map((section, index) => {
    const comparisonIds = section.comparisonIds.filter((id) =>
      validComparisonIds.has(id),
    );

    return {
      title: section.title,
      body: normalizeNarrativeField(section.body),
      comparisonIds:
        comparisonIds.length > 0
          ? comparisonIds
          : [comparisons[index]?.id ?? fallbackId],
    };
  });
}
...
export async function generateMistralProfileAnalysis(
  ...
  const promptInput = buildPromptInput(profile, score, benchmarks, comparisons);
  const systemPrompt = [
  ...
  return {
    ...
    analysis: {
      ...
      deepDiveSections: normalizeDeepDiveSections(
        parsed.deepDiveSections,
        comparisons,
      ),
